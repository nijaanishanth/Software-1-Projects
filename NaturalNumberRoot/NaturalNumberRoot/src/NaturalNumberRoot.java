...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        // set too high value and determine difference
        tooHigh.copyFrom(n);
        tooHigh.increment();
        diff.copyFrom(tooHigh);
        diff.subtract(lowEnough);
        // while loop till the root is found
        while (diff.compareTo(one) > 0) {
            mid.copyFrom(tooHigh);
            mid.add(lowEnough);
            mid.divide(two);
            guess.copyFrom(mid);
            guess.power(r);
            // check to see whether guess is higher or lower
            if (n.compareTo(guess) < 0) {
                tooHigh.copyFrom(mid);
            } else {
                lowEnough.copyFrom(mid);
            }
            diff.copyFrom(tooHigh);
            diff.subtract(lowEnough);
        }
